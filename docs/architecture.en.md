...
Active execution generations, model-owner leases, tool mailboxes, idempotent result digests, and run-bound manual approval decisions are PostgreSQL-backed. Worker and run-bound `on-risk` approval requests may reach any API replica; standalone `/v1/agent-sessions` approvals still belong to the serving API process. Graceful API shutdown relinquishes ownership. A lost owner is superseded by a new generation and the protected S3/PostgreSQL Agent session is recovered before model execution resumes. Unconfirmed calls are never replayed: the old approval is closed as `interrupted/deny`, and the recovered model must issue a fresh call and approval. Non-secret provider metadata is restored from PostgreSQL; credentials remain environment/Vault/KMS inputs. The Kind + Calico gate runs two API and two Worker replicas, deletes the exact owner Pod, verifies recovery and evidence export, restarts PostgreSQL, and rechecks the result. The path remains experimental because a repository acceptance environment is not a production availability or isolation certification.
