...
- v0.1.0 发布范围仅包含 source tarball、Node 22 macOS arm64/x64 portable 包和 `ghcr.io/muniu-ai/muniu:v0.1.0` API/demo 镜像；v0.1.0 不发布或启用桌面运行时 updater，也不把 unsigned APP/ZIP/DMG、Homebrew cask 草案或 Tauri 桌面二进制作为公开制品。仓库仍保留中文安装/卸载/安全文档、Tauri panic log hook、桌面/CLI diagnostics export 及 packaged daemon/Keychain/deep-link 的本地验证能力；Developer ID 签名、Apple 公证、Gatekeeper、真实 updater 下载/安装、packaged crash 写入和双架构干净机证据属于后续生产化范围。
